...
def get_liked():
    ...
    for i in val.json()["items"]:
        indi = {
            ...
            "images": {
                "small": i["track"]["album"]["images"][2]["url"],
...
def getalbums():
    details = []
    header = {"Authorization": f"Bearer {value['access_token']}"}
    url = "https://api.spotify.com/v1/me/top/tracks?limit=8"
    val = requests.get(url, headers=header)
    details = []
    for i in val.json()["items"]:
        data = {
            "images": {
                "small": i["album"]["images"][2]["url"],
                "medium": i["album"]["images"][1]["url"],
                "large": i["album"]["images"][0]["url"],
            },
            "album": i["album"]["name"],
        }
        details.append(data)
